...
// Update ticket number (every 5 minutes)
...
function startQRGeneration() {
    // Generate initial QR and ticket number
    generateQR();
    updateTicketNumber();
    
    // Blur QR code after 5 seconds
    setTimeout(() => {
        blurQRCode();
    }, 5000);
    ...
    qrUpdateInterval = setInterval(() => {
        ...
        // Blur the new QR code after 5 seconds
        setTimeout(() => {
            blurQRCode();
        }, 5000);
    }, 600000); // 10 minutes
    
    // Update ticket number every 5 minutes (300000 ms)
    const ticketUpdateInterval = setInterval(() => {
        if (document.getElementById('qr-page').classList.contains('active')) {
            updateTicketNumber();
            console.log('Ticket number updated at:', new Date().toLocaleTimeString());
        } else {
...
function initializeSections() {
    ...
    const cardInfoArrow = document.getElementById('card-info-arrow');
    if (cardInfoContent && cardInfoArrow) {
        cardInfoContent.classList.remove('open');
        cardInfoArrow.classList.remove('rotated');
        cardInfoArrow.textContent = '▼';
    }
    
    // Control and validation section starts closed
    const controlContent = document.getElementById('control-validation-content');
    const controlArrow = document.getElementById('control-validation-arrow');
    if (controlContent && controlArrow) {
        controlContent.classList.remove('open');
        controlArrow.classList.remove('rotated');
        controlArrow.textContent = '▼';
    }
}

// Blur QR code and show "Create new" button
function blurQRCode() {
...
// Refresh QR code function (disabled - does nothing)
function refreshQR() {
    // Button does nothing - QR stays blurred
    console.log('Create new button clicked - no action taken');
}
